...
p <- ggplot(data=murders) # associate with data
# murders %>% ggplot()
...
h <- heights%>%
  filter(sex=='Male') %>%
  ggplot(aes(x = height))
hist <- h + geom_histogram(binwidth = 10, fill = "blue", col = "black") +
  xlab("Male height in inches") +
  ggtitle("Histogram")

# density plot
dens <- h + geom_density() + ggtitle("Histogram")

# Q-Q plot
h <- heights %>% filter(sex=="Male") %>%
  ggplot(aes(sample=height))

h + geom_qq()# default avg/sd=1

# QQ-plot against a normal distribution with same mean/sd as data
params <- heights %>%
  filter(sex == "Male") %>%
  summarize(mean = mean(height), sd = sd(height))
qq <- h + geom_qq(dparams = params) +
  geom_abline() + ggtitle("Q-Q Plot")
...
# dot placeholder - return single number/vector instead of dataframe
